...
export default class Cluster<JobData = any, ReturnData = any> extends EventEmitter {
    ...
    private async init() {
        const chromeLauncher = this.options.chromeLauncher;
        ...
        if (this.options.concurrency === Cluster.CONCURRENCY_PAGE) {
            this.browser = new builtInConcurrency.Page(browserOptions, puppeteer, chromeLauncher);
        } else if (this.options.concurrency === Cluster.CONCURRENCY_CONTEXT) {
            this.browser = new builtInConcurrency.Context(browserOptions, puppeteer, chromeLauncher);
        } else if (this.options.concurrency === Cluster.CONCURRENCY_BROWSER) {
            this.browser = new builtInConcurrency.Browser(browserOptions, puppeteer, chromeLauncher);
        } else if (typeof this.options.concurrency === 'function') {
            this.browser = new this.options.concurrency(browserOptions, puppeteer, chromeLauncher);
        } else {
